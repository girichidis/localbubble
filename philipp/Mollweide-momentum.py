# create Mollweide projections of the momentum
...
for files in args.files:
    ...
    momentum_map    = np.zeros(NPIX) # x-component of the momentum
    ...
    for i in tqdm(range(Ncell)):
        ...
        prad = sp[("gas", "cell_mass")][i].v * ( sp[("gas", "velocity_x")][i].v * vec_norm_x[i] + \
                                                 sp[("gas", "velocity_y")][i].v * vec_norm_y[i] + \
                                                 sp[("gas", "velocity_z")][i].v * vec_norm_z[i] )
        momentum_map[pix]  += prad

        total_pabs += np.abs(prad)
    ...
    data["total_pabs"] = total_pabs
    
    for name, field, norm in zip(["coldens", "momentum"], \
                            [coldens_map, momentum_map], \
                            [True, True]):
